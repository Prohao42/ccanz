...
"
  ...
  case "$choice" in
    1)
      ...
      ;;
    2)
      read -r -p '  粘贴你的 Anthropic API 密钥: ' api_key
      if [ -n "$api_key" ]; then
        mkdir -p "$HOME/.claude"
...
"
  ...
  case "$choice" in
    ...
    4) read -r -p '  输入模型名称（如 claude-sonnet-4-20250514）: ' model ;;
...
# 模型配置
read -r -p '  是否配置 Claude Code 使用的模型？[y/N]: ' do_model
